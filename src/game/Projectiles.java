...
class Bullet {
	...
	public Bullet(double depX, double depY, double finX, double finY, double vitesse, double degats, double range, Game game){
		this.game = game;
		...
		AnimationTimer anim = new AnimationTimer() {
			public void handle(long arg0) {
				...
				if ((new Point(cercle.getCenterX(), cercle.getCenterY()).ptsDans(game.l_l_triangles, game.l_obs, game.pan) != null) | 
						...
						dep.distance(new Point(cercle.getCenterX(), cercle.getCenterY()))  >  range) {
					game.pan.getChildren().remove(cercle);
					this.stop();
				}
			}
